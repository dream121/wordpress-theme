    ...
    switch ($type) {
      case 'text':
        if (isset($args['color']) && $args['color']) {
          printf("\n".'<p><input class="meteor-color" data-default-color="%s" type="text" id="option-%s" name="%s[%s]" autocomplete="off" tabindex="1" value="%s" /><span class="color-box"></span></p>', $args['default'], $id, $ckey, $id, $val);
        } else {
