...
class Theme {
  ...
  /* Get file path for wordpress attachment url */
  ...
  function render_layout() {
    if (is_singular() && post_password_required() && $this->option('password_protect_behavior') == 'layout') {
      echo '<br/>';
      $this->open_section();
      the_content();
      $this->close_section();
  ...
  function get_post_metadata($keys, $array=false) { global $post;
    ...
    $out = array();
    $post = get_post(); // Set by main query (this allows drafts to be previewed)
...
' . __("This page has no layout assigned.", "theme") . '&nbsp; <a target="_blank" href="' . $config_url . '">' . __("Add one", "theme") . '</a>.
  ...
  */
  ...
  function adjacent_post_links() {
    ...
    $in_same_cat = apply_filters('theme_nextprev_in_same_cat', false);
    $exclude_cats = apply_filters('theme_nextprev_exclude_cats', array());
    
    if (is_attachment()) {
      $prev_post = get_post(get_post()->post_parent);
    } else {
      $prev_post = get_adjacent_post($in_same_cat, $exclude_cats, true);
    }
    $next_post = get_adjacent_post($in_same_cat, $exclude_cats, false);
    return array(
