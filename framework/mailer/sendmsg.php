<?php
  ...
  $from_name_token = sprintf('{{%s}}', $from_name);
  $from_email_token = sprintf('{{%s}}', $from_email);
  
  $from_name = do_template($from_name_token, $all_labels);
  $from_email = do_template($from_email_token, $all_labels);
  
  if ($from_name === $from_name_token) $from_name = null;
  if ($from_email === $from_email_token) $from_email = null;
...
' . $message . '
  ...
  function mailer_send($args) {
    ...
    // Provides: $from_name, $from_email, $recipient, $message, $cc, $bcc, $success
    extract($args);
    ...
    if (empty($from_name)) $from_name = __("Anonymous Sender", "theme");
    if (empty($from_email)) $from_email = $wp_email;
    
    if (!is_valid_email($from_email)) die("ERR: " . __("Invalid email address", "theme"));

    if (empty($success)) $success = __("Your message has been sent!", "theme");
