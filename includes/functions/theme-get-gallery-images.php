<?php
  ...
  function theme_gallery_standalone($arr=array(), $args) { global $der_framework;
    ...
    foreach ($gallery_images as $k => $v) {
      if (isset($v['title'])) $gallery_images[$k]['title'] = $der_framework->shortcode($v['title']);
      if (isset($v['description'])) $gallery_images[$k]['description'] = $der_framework->shortcode($v['description']);
    }
  ...
  function theme_get_gallery_images($args) { global $der_framework;
    ...
    // Allow shortcodes on title & description
    foreach ($out as $k => $v) {
      if (isset($v['title'])) $out[$k]['title'] = $der_framework->shortcode($v['title']);
      if (isset($v['description'])) $out[$k]['description'] = $der_framework->shortcode($v['description']);
    }
