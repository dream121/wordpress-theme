...
<p><strong>NOTE:</strong> If you want to display the contents of a specific page on your Homepage Layout, you can use the <strong>Page Query Override</strong> component on the <a href="admin.php?page=layout-editor">Layout Editor</a>.</p>
  ...
  function theme_get_logo_data($context, $options) { global $der_framework;
    ...
    if ($context == 'theme-options') {
      ...
      if ($logo_image && $logo_image != $der_framework->option('logo_image')) {
        $info = @getimagesize($der_framework->get_upload_path($options['logo_image']));
        if ($info) {
      ...
      if (!empty($retina_logo_image) && $retina_logo_image != $der_framework->option('retina_logo_image')) {
        $info = @getimagesize($der_framework->get_upload_path($options['retina_logo_image']));
        if ($info) {
