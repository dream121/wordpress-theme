<?php
  ...
  function meteor_page_override($atts, $content='', $code='') { global $der_framework;
    ...
    if ($id) {
      ...
      $der_framework->page_override_id = $id;
      
      $wp_query = new WP_Query(sprintf('post_type=page&p=%d', $id));
  ...
  function meteor_single_post($atts, $content='', $code='') { global $der_framework;
    ...
    $post = $args['posts'][0]; // This is not the global $post object, just the array containing data from component_get_posts

    if (isset($der_framework->page_override_id)) {
      $p = get_post($der_framework->page_override_id); // Needed for page override to work
    } else {
      $p = get_post(); // Post is set by query, this allows drafts to be previewed
    }
...
<!-- + -->', $post);
    ...
    if (!$is_attachment && $args['show_post_tags']) {
      ...
      $terms = get_the_terms($p->ID, get_post_type() == 'portfolio' ? 'portfolio-tag' : 'post_tag');
